...
def build_body_block_from_template(
    ...
    body_index: int,
    template_path: str,
    wing_dihed_deg: float = 0.0,   # NEW, default keeps old behavior
) -> List[str]:
    ...
    # Decide part_phi based on cowling name and wing dihedral
    group_name = body.get("group_name", "")

    phi_deg = 0.0
    if "LF_Cowling" in group_name:
        # Left cowling: same sign as wing dihedral
        phi_deg = float(wing_dihed_deg)
    elif "RT_Cowling" in group_name:
        # Right cowling: opposite sign so it tilts with the wing
        phi_deg = -float(wing_dihed_deg)
    ...
    for line in template_lines:
        ...
        if stripped.startswith("P _body/b/_part_rad"):
            ...
            continue
        if stripped.startswith("P _body/b/_part_phi"):
            # NEW: tilt cowlings by wing dihedral; others stay 0
            out_lines.append(f"P _body/{b}/_part_phi {phi_deg:.9f}")
            continue
